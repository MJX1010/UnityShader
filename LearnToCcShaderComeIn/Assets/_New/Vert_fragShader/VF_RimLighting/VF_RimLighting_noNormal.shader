Shader "学习Shader/V&F/VF_RimLighting_noNormal"
{
	//面板显示属性
	properties
	{
		...
		//凹凸纹理
		//_BumpMap("BumpMap", 2D) = "bump"{}//此着色器不启用
		//主颜色
...
#pragma target 3.0
				...
				/*一般输入结构体都是声明一些需要使用到的数据结构*/
					...
					//【6】变量声明
					//系统光照颜色
					uniform float4 _LightColor0;//大小写？？
					...
					v2f vert(appdata v)
					{
						...
						//o.normal = mul(_World2Object, float4(v.normal, 0)).xyz;
						//获取模型顶点在世界空间中的位置坐标
					...
					fixed4 frag(v2f i) : SV_Target//也可使用Color，推荐使用SV_Target
					{
						...
						float3 Emissive = _RimColor.rgb * pow(Rim, _RimPower) * _RimIntensity;
							...
							//     
							float3 finalColor = Diffuse * tex2D(_TextureDiffuse, TRANSFORM_TEX(i.texcoord.xy, _TextureDiffuse)).rgb * _MainColor.rgb + Emissive;
